# Stratégie :
#   Phase 1 : Exploration large (Latin Hypercube, 50 combinaisons)
#   Phase 2 : Zoom local autour du meilleur (grille resserrée)
#
# Référence : https://www.tmwr.org/grid-search.html
#
# Prérequis : objets issus de 00–04
#
# Objets exportés :
#   xgb_tune_results, xgb_zoom_results   — résultats des 2 passes
#   xgb_best_params                      — hyperparamètres finaux
#   xgb_final_wflow                      — workflow prêt pour last_fit()
#   plot_xgb_tuning, plot_xgb_zoom       — visualisations
#   plot_xgb_importance                  — importance des variables
# ==============================================================================
...
if (!requireNamespace("lme4", quietly = TRUE)) {
  stop(
    "Le package 'lme4' est requis par tune_race_anova().\n",
    "Installez-le avec : install.packages('lme4')",
    call. = FALSE
  )
}
...
# ── 3. Configuration parallèle (une seule fois) ─────────────────────────────
...
if (file.exists(rds_p1)) {
  ...
  set.seed(2026)
  message("→ Tuning XGBoost passe 1 en cours...")
...
message("\nMeilleurs hyperparamètres (passe 1) :")
print(xgb_best_p1)
...
# PHASE 2 : ZOOM LOCAL
#
# Grille resserrée autour des valeurs optimales de la phase 1.
# Les ranges sont ajustés manuellement après inspection des résultats.
...
if (file.exists(rds_p2)) {
  ...
  set.seed(2026)
  message("→ Tuning XGBoost passe 2 (zoom) en cours...")
...
message("\nMeilleurs hyperparamètres (final) :")
...
plot_xgb_tuning <- xgb_tune_results |>
  ...
  labs(
    title    = "Tuning XGBoost — Passe 1 (exploration)",
...
# ── Phase 2 : visualisation adaptée au LHS ───────────────────────────────────
...
rm(list = setdiff(ls(), c(
  # Données de base
  "data", "churn_split", "train_data", "test_data", "churn_folds",
  "recipe_xgb", "churn_metrics",
  # Tuning
  ...
  "xgb_final_wflow",
  # Visualisations
  "plot_xgb_tuning", "plot_xgb_zoom",
  "xgb_fit_train", "plot_xgb_importance",
  # Contrôle
  "race_ctrl"
)))
